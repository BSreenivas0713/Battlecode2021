...
import musketeerplayerpostqual.Comms.*;
import musketeerplayerpostqual.Util.*;
import musketeerplayerpostqual.Debug.*;
import musketeerplayerpostqual.fast.FastIterableIntSet;
import musketeerplayerpostqual.fast.FastIterableLocSet;
import musketeerplayerpostqual.fast.FastLocIntMap;
import musketeerplayerpostqual.fast.FastIntLocMap;
import musketeerplayerpostqual.fast.FastQueue;
import musketeerplayerpostqual.fast.FastIntIntMap;
...
public class EC extends Robot {
    ...
    public int getPoliticianInfluence() throws GameActionException {
        if (currInfluence > 1500 && numPols % Util.nullRushPolFreqHighInfluence == 0) {
            return Math.max(100, currInfluence / 20);
        } else if (numPols % Util.nullRushPolFreqLowInfluence == 0 && currInfluence > 400) {
            return Math.max(100, currInfluence / 20);
        } else {
    ...
    public void makePolitician() throws GameActionException {
        if ((currInfluence > 1500 && numPols % Util.nullRushPolFreqHighInfluence == 0) ||
        (numPols % Util.nullRushPolFreqLowInfluence == 0 && currInfluence > 400)) {
            RushFlag rushFlag = ECflags.peek();
            if (rushFlag != null) {
                Debug.println(Debug.info, "making null rusher, but with destination from ECflags: x: " + rushFlag.dx + home.x + "; y: " + rushFlag.dy + home.y);
            ...
            else if (enemyECsFound.size > 0) {
                MapLocation enemyEC = enemyECsFound.locs[0];
                Debug.println(Debug.info, "making null rusher, but with destination from enemyECsFound: x: " + enemyEC.x + "; y: " + enemyEC.y);
                nextFlag = Comms.getFlagRush(InformationCategory.ENEMY_EC, (int)(4 * Math.random()), Comms.GroupRushType.MUC_POL, 
                                            enemyEC.x - home.x + Util.dOffset, enemyEC.y - home.y + Util.dOffset);
            }
            else {
                Debug.println(Debug.info, "making regular null rusher");
    ...
    public boolean trySendARush() throws GameActionException {
        ...
        if(buildRobot(toBuild, influence)) {
            ...
            int dx = Math.abs(home.x - enemyLocation.x);
            int dy = Math.abs(home.y - enemyLocation.y);
            int cooldown = Math.min(Util.maxRushCooldown, Util.baseRushCooldown + Math.max(dx, dy));
